	...
	if(ok){
		str = "<div id=\"selectdiv\"><label class=\"goodsname\">"+goodsname+"</label>"+
				"<label class=\"goodscount\"><input class=\"finalresult\" goodsid=\""+id+"\" rprice=\""+rprice+"\" type=\"text\" value=\"1\"/></label>"+
						"<label class=\"del\" rprice=\""+rprice+"\"><img src=\""+path+"/statics/img/cancel-on.png\"/></label>"+
...
$("#m_goodsPackCode").blur(function(){
	...
	if(mgpc != ""){
		$.post(path+"/backend/goodspackcodeisexit.shtml",{'goodsPackCode':mgpc,'id':$("#m_id").val()},function(result){
			if(result == "repeat"){
