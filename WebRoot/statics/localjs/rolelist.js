...
//全局变量:获取路径值${pageContext.request.contextPath}
  var path=$("#path").val();
...
function loadingRoleList(){
	window.location.href = path+"/backend/rolelist.html";
}
...
$("#addRoleBtn").click(function(){
	...
	if(role.roleCode == "" || role.roleCode == null){
		...
		$.ajax({
			url: path+'/backend/addRole.shtml',
			type: 'POST',
...
$(".modifyrole").click(function(){
	...
	if(roleCode == "" || roleCode == null){
		...
		if(confirm(tip)){ //提示的confirm弹窗被“确认”
			var role = new Object();
			role.id = id;
			role.roleCode = roleCode;
			role.roleName = roleName;
			$.ajax({
				url: path+'/backend/modifyRole.shtml',
				type: 'POST',
				data:{role:JSON.stringify(role)},
				dataType: 'html',
				timeout: 1000,
...
$(".delrole").click(function(){
	...
	if(confirm(tip)){
		...
		role.id = id;
		$.ajax({
			url: path+'/backend/delRole.shtml',
			type: 'POST',
			data:{role:JSON.stringify(role)},
			dataType: 'html',
			timeout: 1000,
