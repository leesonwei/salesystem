...
$('.adduser').click(function(e){
	$("#add_formtip").html('');
	e.preventDefault();//该preventDefault()方法将通知 Web 浏览器不要执行与事件关联的默认动作（如果存在这样的动作）
	$('#addUserDiv').modal('show'); //显示modal弹窗
...
$('.viewuser').click(function(e){
	var path = $("#path").val();
	var m_id = $(this).attr('id'); //this表示的是当前对象,即是获取当前对象的id值(id="${user.id})
	$.ajax({
		url: path+'/backend/getuser.shtml',
		type: 'POST',
		data:{id:m_id},
		dataType: 'html',
		timeout: 1000,
		...
		success: function(result){
			...
			if("failed" == result){
				...
				m = eval('(' + result + ')');//eval()函数可计算某个字符串，并执行其中的的 JavaScript代码。返回值 为：通过计算 string 得到的值（如果有的话）。
				$("#v_id").val(m.id);
...
$('.deluser').click(function(e){
	...
	if(confirm("您确定要删除【"+d_logincode+"】这个用户吗？")){
		//delete
		$.post(path+"/backend/deluser.shtml",{'delId':d_id,'delIdCardPicPath':d_idcardpicpath,'delBankPicPath':d_bankpicpath,'delUserType':d_usertype},function(result){
			if("success" == result){
				alert("删除成功！");
				window.location.href=path+"/backend/userlist.html";
...
$("#m_roleId").change(function(){
	...
	if(sel_role == 2){
		$.post(path+"/backend/loadUserTypeList.shtml",{'s_role':sel_role},function(result){
			if(result != ""){
				for(var i=0;i<result.length;i++){
					$("#m_selectusertype").append("<option value=\""+result[i].valueId+"\">"+result[i].valueName+"</option>");
				}
...
$("#a_logincode").blur(function(){
	...
	if(alc != ""){
		$.post(path+"/backend/logincodeisexit.shtml",{'loginCode':alc,'id':'-1'},function(result){
		 // $.post(path+"/backend/logincodeisexit.shtml",{'loginCode':alc},function(result){
			if(result == "repeat"){
...
$("#m_logincode").blur(function(){
	...
	if(mlc != ""){
		$.post(path+"/backend/logincodeisexit.shtml",{'loginCode':mlc,'id':$("#m_id").val()},function(result){
		//  $.post(path+"/backend/logincodeisexit.shtml",{'loginCode':mlc},function(result){
			if(result == "repeat"){
...
function delpic(id,closeSpan,uploadBtn,obj,picpath,picText,fileinputid){
	var path=$("#path").val();
	//delete
	$.post(path+"/backend/delpic.shtml",{'id':id,'picpath':picpath},function(result){
		if("success" == result){
			alert("删除成功！");
			$('#'+picText).val('');
...
{  
	...
	if($("#"+t1+"").val() == '' || $("#"+t1+"").val() == null){
		...
		$.ajaxFileUpload
	    ({ 
	           url:path+'/backend/upload.shtml', //处理上传文件的服务端
	           secureuri:false,
	           fileElementId:t1,
	           dataType: 'json',
